...
if strmatch('all',filetype),
	filetype = {'bem','orig','smoothwm','white','pial','inflated','curv','thickness','aparc','xfm','sphere'};
end
...
for f = filetype,
	...
	switch f,
		...
		case {'sphere'},
			...
			% load sphere
			file.lh.sphere = fullfile(surfPath,'lh.sphere');
			if exist(file.lh.sphere) == 2,
				[lh.sphere.vertices, lh.sphere.faces] = freesurfer_read_surf(file.lh.sphere);
			else
				msg = sprintf('file does not exist: %s',file.lh.sphere);
				warning(msg);
			end

			% load sphere
			file.rh.sphere = fullfile(surfPath,'rh.sphere');
			if exist(file.rh.sphere) == 2,
				[rh.sphere.vertices,rh.sphere.faces] = freesurfer_read_surf(file.rh.sphere);
			else
				msg = sprintf('file does not exist: %s',file.rh.sphere);
				warning(msg);
			end
		...
		case {'aparc'},
			...
			if exist(file.lh.aparc) == 2,
				lh.aparc = read_annotation(file.lh.aparc);
			else
			...
			if exist(file.rh.aparc) == 2,
				rh.aparc = read_annotation(file.rh.aparc);
			else
