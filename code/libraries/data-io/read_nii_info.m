function data =read_nii_info(fn)


niistruct =nii_tool('load',fn);   
grayCoordinateData =  squeeze(niistruct.img)'; 
...
for i=1:200
    ...
    id = strfind(line_ex,'IndexOffset=');
    
    
    if(~isempty(id))
        scanstr = textscan(line_ex,'<BrainModel IndexOffset="%d" IndexCount="%d"');
        offcnt(cnt,:) = cell2mat(scanstr);
        line_ex = fgetl(fid);
        scanstr = textscan(line_ex,['<VertexIndices>' repmat('%d ',[1,offcnt(cnt,2)]) '</VertexIndices>']);
        vid{cnt} = cell2mat(scanstr) + 1;
        if(cnt==2)
            break
        end
        cnt=cnt+1;
...
% data.cdata = grayCoordinateData;
data.offset = offcnt(:,1)+1;
data.vertexNum = offcnt(:,2) ;
data.vertexInd = vid;
...
if isempty(cmd)
    ...
    if ischar(cmd)
        if ispc
            cmd = @(nam) sprintf('start "" /B %s -nf "%s"', cmd, nam);
        else
