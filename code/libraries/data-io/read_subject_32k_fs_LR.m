...
function subject = read_subject_32k_fs_LR(subjectpath)
% 
...
feature_names= {'inflated', 'sphere', 'pial', 'white', 'midthickness', 'very_inflated', ...
    'thickness', 'curvature', 'aparc', 'sulc', 'BA', 'pial_MSMAll', 'white_MSMAll'};
...
for i =1:length(feature_names)
    ...
    for j= 1: length(feature_types)
        try
            g=gifti(sprintf('%s/%s.L.%s.32k_fs_LR.%s.gii', subjectpath, subjectid, feature_names{i}, feature_types{j}));
            if (isfield(g,'cdata'))
                g= g.cdata;
            end
            eval(sprintf('subject.lh.%s = g;', feature_names{i}));
            
            g=gifti(sprintf('%s/%s.R.%s.32k_fs_LR.%s.gii', subjectpath, subjectid, feature_names{i}, feature_types{j}));
            if (isfield(g,'cdata'))
                g= g.cdata;
            end
