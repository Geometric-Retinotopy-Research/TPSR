...
%
%%
function [NRMSE_raw, NRMSE_new, R2_raw,R2_new, AIC_raw, AIC_new, extra]= compute_fMRI_metrics_32k_v2(subjectfn, lr, hat_vis, roiid)
...
for i=1:6
    ...
    function [R2,modelfit,fmridata] = R2bypara(parameters)
        for i=1:6
    ...
    function [R2 , modelfit, fmridata, ...
        ...
        [nR2,modelfit,fmridata] = R2bypara([x,y,parai(3:5)]);
        R2 = -nR2;
        if R2>100
            R2 =0;
        end
        ...
        para35 =  parai(3:5);
        x0 = fminsearch(@(para35)R2bypara([xhat,yhat,para35]), para35);    
        
        [nR2_new,modelfit_new,~] = R2bypara([xhat,yhat,x0]);
        R2_new = -nR2_new;
        if R2_new>100
            R2_new =0;
        end
